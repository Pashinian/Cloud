...
public class ControladorCoche {
	...
	@Autowired
	private GestorCoche gestorUsuario;
	...
	//a la vista
	@PostMapping("/doLogin")
	public String login(@RequestParam("Modelo") String modelo, 
				@RequestParam("Caballos") String caballos,
				Model model) {
		
		System.out.println("Entrando login");
		Coche vehiculo = new Coche(0, caballos, caballos);
		vehiculo.setModelo(modelo);
		vehiculo.setCaballos(caballos);
		
		boolean validado = gestorUsuario.validar(vehiculo);
